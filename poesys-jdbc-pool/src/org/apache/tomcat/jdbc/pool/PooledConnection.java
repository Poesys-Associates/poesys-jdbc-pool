...
public class PooledConnection {
  ...
  /** max_retries property name */
  public static final String PROP_MAX_RETRIES = PoolUtilities.PROP_MAX_RETRIES;
  ...
  public boolean checkUser(String username, String password) {
    ...
    result =
      result
          && ((password == null && storedPwd == null) || (password != null && password.equals(storedPwd)));
   ...
   * @throws SQLException if a call to
  ...
  protected void connectUsingDataSource() throws SQLException {
    ...
    if (poolProperties.getDataSource() instanceof javax.sql.XADataSource) {
      ...
      throw new SQLException("DataSource is of unknown class:"
                             + (poolProperties.getDataSource() != null ? poolProperties.getDataSource().getClass()
                                 : "null"));
    }
  }
  ...
  protected void connectUsingDriver() throws SQLException {
    ...
    if (getAttributes().containsKey(PROP_MAX_RETRIES)) {
      maxRetries = new Integer((String)getAttributes().get(PROP_MAX_RETRIES));
    } else {
      maxRetries = poolProperties.getMaxConnectionRetries();
      getAttributes().put(PROP_MAX_RETRIES, maxRetries);
    }
    ...
    while (retries < maxRetries) {
      try {
        ...
        log.debug("Retry " + retries + " after recoverable exception");
      } catch (Exception x) {
        if (log.isDebugEnabled()) {
   ...
   * 
   * @param finalize if set to true, a call to
   ...
   * 
   * @param validateAction the action used. One of {@link #VALIDATE_BORROW},
   *          {@link #VALIDATE_IDLE}, {@link #VALIDATE_INIT} or
   *          {@link #VALIDATE_RETURN}
   * @param sql the SQL to be used during validation. If the
   *          {@link PoolConfiguration#setInitSQL(String)} has been called with
   *          a non null value and the action is {@link #VALIDATE_INIT} the init
   *          SQL will be used for validation.
   * 
   * @return true if the connection was validated successfully. It returns true
   *         even if validation was not performed, such as when
  ...
  /**
   * Returns the stack trace from when this connection was borrowed. Can return
   * null if no stack trace was set.
   * 
   ...
   * 
   * @param timestamp the timestamp as defined by
  ...
  /**
   * An interceptor can call this method with the value true, and the connection
   * will be closed when it is returned to the pool.
   * 
   * @param discarded - only valid value is true
   * @throws IllegalStateException if this method is called with the value false
   *           and the value true has already been set.
   */
   ...
   * 
   * @param lastValidated a timestamp as defined by
   *          {@link System#currentTimeMillis()}
   */
  ...
  /**
   * Sets the pool configuration for this connection and connection pool. Object
   * is shared with the {@link ConnectionPool}
   * 
   ...
   * 
   * @return the timestamp of the last successful validation query execution as
   *         defined by {@link System#currentTimeMillis()}
   */
   ...
   * {@link PoolConfiguration#getUseLock()} return true. The per connection lock
   * ensures thread safety is multiple threads are performing operations on the
   * connection. Otherwise this is a noop for performance
   */
  public void lock() {
