...
public class PoolProperties implements PoolConfiguration, Cloneable,
  ...
  private volatile String name =
    "Tomcat Connection Pool[" + (poolCounter.addAndGet(1)) + "-"
  ...
  private volatile int maxConnectionRetries = 5;
  ...
  public InterceptorDefinition[] getJdbcInterceptorsAsArray() {
    if (interceptors == null) {
      if (jdbcInterceptors == null) {
        ...
        InterceptorDefinition[] definitions =
          new InterceptorDefinition[interceptorValues.length + 1];
        ...
        for (int i = 0; i < interceptorValues.length; i++) {
          ...
          if (propIndex < 0 || endIndex < 0 || endIndex <= propIndex) {
            definitions[i + 1] =
              new InterceptorDefinition(interceptorValues[i].trim());
          } else {
            ...
            String propsAsString =
              interceptorValues[i].substring(propIndex + 1,
                                             interceptorValues[i].length() - 1);
            ...
            for (int j = 0; j < props.length; j++) {
              ...
              definitions[i + 1].addProperty(new InterceptorProperty(propName,
                                                                     propValue));
            }
          }
        }
  ...
  public void setValidatorClassName(String className) {
    ...
    try {
      ...
      log.warn("The class " + className
               + " does not implement the Validator interface.", e);
    } catch (InstantiationException e) {
      log.warn("An object of class " + className
               + " cannot be instantiated. Make sure that "
               + "it includes an implicit or explicit no-arg constructor.", e);
    } catch (IllegalAccessException e) {
      log.warn("The class " + className
               + " or its no-arg constructor are inaccessible.", e);
    }
  }
    ...
    try {
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        for (int j = 0; j < prefix.length; j++) {
          ...
          String name =
            prefix[j] + fields[i].substring(0, 1).toUpperCase(Locale.US)
  ...
  public boolean isPoolSweeperEnabled() {
    ...
    boolean result =
      timer && (isRemoveAbandoned() && getRemoveAbandonedTimeout() > 0);
    result = result || (timer && getSuspectTimeout() > 0);
    result =
      result || (timer && isTestWhileIdle() && getValidationQuery() != null);
      ...
      if (clazz == null) {
        if (getClassName().indexOf(".") < 0) {
          if (log.isDebugEnabled()) {
            log.debug("Loading interceptor class:"
                      + PoolConfiguration.PKG_PREFIX + getClassName());
          }
          clazz =
            Class.forName(PoolConfiguration.PKG_PREFIX + getClassName(),
                          true,
