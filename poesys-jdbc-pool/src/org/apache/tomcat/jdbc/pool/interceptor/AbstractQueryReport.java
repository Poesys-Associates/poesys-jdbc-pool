...
public abstract class AbstractQueryReport extends
   ...
   * @param name the name of the method used to execute
   *          {@link AbstractCreateStatementInterceptor#isExecute(Method, boolean)}
   * @param start the time the query execution started
   * @param t the exception that happened
   * @return - the SQL that was executed or the string &quot;batch&quot; if it
   *         was a batch execution
   */
  protected String reportFailedQuery(String query, Object[] args,
                                     final String name, long start, Throwable t) {
    // extract the query string
    String sql =
      (query == null && args != null && args.length > 0) ? (String)args[0]
          : query;
   ...
   *         was a batch execution
   */
  protected String reportQuery(String query, Object[] args, final String name,
  ...
  /**
   * returns the query measure threshold. This value is in milliseconds. If the
   * query is faster than this threshold than it wont be accounted for
   * 
  ...
  /**
   * Sets the query measurement threshold. The value is in milliseconds. If the
   * query goes faster than this threshold it will not be recorded.
   * 
  ...
  protected Constructor<?> getConstructor(int idx, Class<?> clazz)
    ...
    if (constructors[idx] == null) {
      Class<?> proxyClass =
        Proxy.getProxyClass(SlowQueryReport.class.getClassLoader(),
                            new Class[] { clazz });
      constructors[idx] =
  ...
  public Object createStatement(Object proxy, Method method, Object[] args,
    ...
    try {
      ...
      result =
        constructor.newInstance(new Object[] { new StatementProxy(statement,
                                                                  sql) });
      return result;
    } catch (Exception x) {
