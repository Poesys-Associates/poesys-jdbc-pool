 ...
 * database. The {@link org.apache.tomcat.jdbc.pool.ConnectionPool} is optimized
 * to do as little work as possible. The pool itself doesn't remember settings
 * like {@link java.sql.Connection#setAutoCommit(boolean)},
 * {@link java.sql.Connection#setReadOnly(boolean)},
 * {@link java.sql.Connection#setCatalog(String)} or
 * {@link java.sql.Connection#setTransactionIsolation(int)}. It relies on the
 * application to remember how and when these settings have been applied. In the
 * cases where the application code doesn't know or want to keep track of the
 * state, this interceptor helps cache the state, and it also avoids roundtrips
...
public class ConnectionState extends JdbcInterceptor {
  ...
  protected final String[] readState = { "getAutoCommit",
                                        "getTransactionIsolation",
                                        "isReadOnly", "getCatalog" };
  protected final String[] writeState = { "setAutoCommit",
                                         "setTransactionIsolation",
                                         "setReadOnly", "setCatalog" };
  ...
  public void reset(ConnectionPool parent, PooledConnection con) {
    ...
    if (poolProperties.getDefaultTransactionIsolation() != DataSourceFactory.UNKNOWN_TRANSACTIONISOLATION) {
      try {
        ...
        transactionIsolation = null;
        log.error("Unable to reset transaction isolation state to connection.",
                  x);
      }
    }
    ...
    if (poolProperties.getDefaultAutoCommit() != null) {
      try {
        if (autoCommit == null
            || autoCommit.booleanValue() != poolProperties.getDefaultAutoCommit().booleanValue()) {
    ...
    if (poolProperties.getDefaultCatalog() != null) {
      try {
        if (catalog == null
            || (!catalog.equals(poolProperties.getDefaultCatalog()))) {
